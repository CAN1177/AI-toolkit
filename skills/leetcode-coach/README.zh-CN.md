...
1. 每次开始新一轮训练前，先检查外部记录仓库配置，并在仓库根目录执行 `git pull`。
2. 训练时读取外部记录目录里的 `pattern-progress.md`，不要把当前仓库 `references/` 里的模板直接当成真实记录。
3. 默认从国内力扣（`leetcode.cn`）选题，并把题目完整给出，不要只做缩写式转述。
4. 进入解题前，先把题目里的关键术语用白话解释清楚。
5. 如果用户卡在某个概念上，先用白话解释、生活类比和最小例子讲清楚，再继续推代码。
6. 验证环节保持轻量，方向已经对了就确认关键点后继续推进。
7. 当前算法类型没讲透、没练完、没得到用户明确确认前，不要切到下一个类型。
8. 只有在用户明确确认当前类型可以收口之后，才更新外部记录目录里的 `pattern-progress.md`；本地记录一旦更新，立刻回到仓库根目录执行 `git add`、`git commit`、`git push`。
...
2. `git pull`、`git add`、`git commit`、`git push` 都在 `LEETCODE_COACH_RECORDS_REPO_PATH` 指向的仓库根目录执行。
3. 真正被读取和写入的记录文件，在默认 `leetcode-coach/` 子目录或 `LEETCODE_COACH_RECORDS_SUBDIR` 指向的子目录里。
4. 如果 `git pull` 失败，就先解决同步问题，不能继续依赖旧的本地记录训练。
5. 如果 `git push` 失败，要明确视为“本地已更新，远端未同步成功”。
